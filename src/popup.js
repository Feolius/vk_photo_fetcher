(function () {
    ...
    const extractChatId = (url) => {
        ...
        if (urlParser.hostname !== "vk.ru") {
            return null;
        }
        const pathParts = urlParser.pathname.split("/");
        // Checking for path urls to match the pattern vk.ru/im/convo/{cid} where {cid} is chat id.
        if (pathParts.length === 4 && pathParts.slice(0, 3).join("/") === "/im/convo") {
            return {chatId: pathParts[3], groupId: null};
        }
        // Checking for path urls to match the pattern vk.ru/gim{gid}/convo/{cid} where {gid} is a group id
        ...
        // Checking for old format vk.ru/im?sel={cid} and for vk.ru/gim{gid}?sel={cid}, where {cid} is a chat id.
